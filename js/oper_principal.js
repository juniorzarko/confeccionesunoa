class Domprincipal {
  ...
  cargarDOM() {
    this.DOMprincipalvar = window.document
    console.log(this.DOMprincipalvar);
    return;
  }

  setDomprincipalvar(){
    return this.DOMprincipalvar;
  }
}
...
class Manejomodulos {
  ...
  consultarOperarioxId(id) {
    $.ajax({
      ...
      // url: "http://localhost:8080/api/Cabin/all",
      url:
        "http://localhost/confeccionesunoa/principal.php/Operario/buscarxid/" +
        id,
      // la información a enviar
      // (también es posible utilizar una cadena de datos)
      //si el metodo del servicio recibe datos, es necesario definir el parametro adicional
  ...
  actualizarOperario(id) {
    ...
    $.ajax({
      ...
      // url: "http://localhost:8080/api/Cabin/all",
      url:
        "http://localhost/confeccionesunoa/principal.php/Operario/actualizar/" +
        id,
      // la información a enviar
      // (también es posible utilizar una cadena de datos)
      //si el metodo del servicio recibe datos, es necesario definir el parametro adicional
      ...
      success: function (respuesta) {
        //escribe en la consola del desarrollador para efectos de depuración
        console.log(
          "entro por success listar de actualizar con valores nuevos"
        );
        //console.log(respuesta);
  ...
  eliminar(id) {
    $.ajax({
      ...
      // url: "http://localhost:8080/api/Cabin/all",
      url:
        "http://localhost/confeccionesunoa/principal.php/Operario/eliminar/" +
        id,
...
class Manejolotes {
  ...
  consultarlotexId(id) {
    $.ajax({
      ...
      // url: "http://localhost:8080/api/Cabin/all",
      url:
        "http://localhost/confeccionesunoa/principal.php/lote/buscarxid/" + id,
      // la información a enviar
      // (también es posible utilizar una cadena de datos)
      //si el metodo del servicio recibe datos, es necesario definir el parametro adicional
      // data : datos,
  ...
  actualizarlotexid(id) {
    ...
    $.ajax({
      url:
        "http://localhost/confeccionesunoa/principal.php/lote/actualizar/" + id,
  ...
  eliminarlote(id) {
    $.ajax({
      ...
      // url: "http://localhost:8080/api/Cabin/all",
      url:
        "http://localhost/confeccionesunoa/principal.php/lote/eliminar/" + id,
...
class ManejOperaciones {
  ...
  consultaroperacionesxId(id) {
    $.ajax({
      ...
      // url: "http://localhost:8080/api/Cabin/all",
      url:
        "http://localhost/confeccionesunoa/principal.php/operaciones/buscarxid/" +
        id,
      // la información a enviar
      // (también es posible utilizar una cadena de datos)
      //si el metodo del servicio recibe datos, es necesario definir el parametro adicional
  ...
  actualizaroperacionesxid(id) {
    ...
    $.ajax({
      url:
        "http://localhost/confeccionesunoa/principal.php/operaciones/actualizar/" +
        id,
  ...
  eliminaroperaciones(id) {
    $.ajax({
      ...
      // url: "http://localhost:8080/api/Cabin/all",
      url:
        "http://localhost/confeccionesunoa/principal.php/operaciones/eliminar/" +
        id,
...
function enviaridoperario(id) {
  //,nombre,apellido,direccion,telefono,cargo,email)
  $("#codigoId").val(id);
...
function validarOperario() {
  ...
  console.log( nombreOperario,apellidOperario, direccionOperario,telefonOperario,cargOperario,emailOperario);
  if (
    nombreOperario === "" ||
    apellidOperario === "" ||
    direccionOperario === "" ||
...
function validarLote() {
  ...
  nombreLote = document.getElementById("nombreLote").value;
  cantidadLote = document.getElementById("cantidadLote").value;
  numeros = /^\d{10}$/; // Solo 10 numeros.
 console.log(codigoLote,nombreLote,cantidadLote);

  // if (  codigoLote === "" ||  nombreLote === "" || cantidadLote === ""  ){
  //   alert("Todos los campos son Obligatorios");
  //   valido = false;
  //   return valido;
  // } else if (codigoLote.length > 10) {
  //   alert("El Numero del Lote es muy largo");
  //   valido = false;
  //   return valido;
  // } else if (!nombreLote.test(nombreOperario)) {
  //   alert("El Nombre del Lotte no es valido");
  //   valido = false;
  //   return valido;
  // } else if (cantidadLote.length > 10) {
  //   alert("La Cantidad del lote es muy largo");
  //   valido = false;
  //   return valido;
  // } else if (isNaN(codigoLote)) {
  //   alert("Solo Números");
  //   valido = false;
  //   return valido;
  // } else if (!numeros.test(codigoLote)) {
  //   alert("Nombre del cargo tiene cracteres Incorrectos");
  //   valido = false;
  //   return valido;
  // } else if (emailOperario.length > 50) {
...
//   input.addEventLstener('keyup' , function() {
//     console.log('tecl levantada');

//   });

// });

// window.addEventListener("load", cargarformu);
//  const expresiones = {
// 	numeros: /^\d{10}$/, // Solo 10 numeros.
